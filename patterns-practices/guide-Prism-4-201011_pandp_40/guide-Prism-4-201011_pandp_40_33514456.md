...
>**Note:** When the shell is instantiated, Silverlight resolves the value of the <strong>Regions:RegionManager.RegionName</strong> attached property and invokes a callback in the <strong>RegionManager</strong> class. This callback creates a region and associates it with the <strong>ItemsControl</strong> control.
...
>**Note:** In this example, the modules are directly referenced by the shell. That is why this example is able to use <strong>typeof(Module)</strong> to add modules to the catalog. But keep in mind that modules whose type is not already available can also be added to the catalog.<br />
    The <strong>WhenAvailable</strong> initialization mode is the default value if no initialization mode is specified.
